"""Contract-compliant Event schema strictly aligned with docs/api-contract.md."""
...
class Event:
    ...
    confidence: float
    evidence_image: str
    source: str = "data_demo_simulator"
    ...
    def validate(self) -> None:
        """Validate payload against official API contract rules."""
        ...
        if self.source != "data_demo_simulator":
            raise ValueError(
                f"Invalid source '{self.source}'. For this simulator, source must be 'data_demo_simulator'."
            )
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert event to dictionary preserving non-null contract fields."""
        data = asdict(self)
        # Prune fields that are None if desired, or keep keys if they were explicitly set
        result: Dict[str, Any] = {}
        # Guarantee field ordering consistent with docs/api-contract.md
        ordered_keys = [
            "event_id",
            "bus_id",
            "event_type",
    ...
    def from_dict(cls, data: Dict[str, Any]) -> Event:
        ...
        return cls(
            ...
            source=data.get("source", "data_demo_simulator"),
