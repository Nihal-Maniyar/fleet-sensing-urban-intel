...
# Configure Ultralytics runtime directory and offline mode safely
os.environ.setdefault("YOLO_CONFIG_DIR", str((Path(__file__).resolve().parent.parent.parent / "runtime" / ".ultralytics").resolve()))
os.environ.setdefault("YOLO_OFFLINE", "true")


def detect_compute_device(preference: Optional[str] = None) -> str:
    """Detect available compute device (cuda, mps, cpu) with fallback."""
    if preference and preference.lower() in ("cpu", "cuda", "mps"):
...
class YOLODetector:
    ...
    def __init__(
        ...
        # 1. Resolve weights path (parameter -> env var -> standard candidate locations)
        ...
        if weights_path is None:
            ...
            for cand in candidates:
                if cand.exists():
                    weights_path = str(cand.resolve())
                    break

        self.weights_path = weights_path

        # 2. Resolve confidence threshold
        if confidence_threshold is None:
            try:
                self.confidence_threshold = float(os.environ.get("MODEL_CONFIDENCE_THRESHOLD", "0.35"))
            except ValueError:
        ...
        self.device = detect_compute_device(device or os.environ.get("DEVICE"))
        self.model = None
    ...
    def _load_weights(self, weights_path: str) -> None:
        ...
        try:
            ...
            try:
                ...
                pass
            logger.info("Loaded custom YOLO model from %s on device %s", weights_path, self.device)
            ...
            try:
                ...
                for r in results:
                    ...
                    for box in boxes:
                        ...
                        raw_cls_name = r.names.get(cls_id, "POTHOLE")
                        cls_name = str(raw_cls_name).upper()
                        # Normalize pothole variants
                        if "POTHOLE" in cls_name or "HOLE" in cls_name:
                            cls_name = "POTHOLE"
                        elif cls_name not in self.target_classes:
                            cls_name = "POTHOLE"
