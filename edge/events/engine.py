...
class TemporalEventEngine:
    ...
    def __init__(
        ...
        # 1. Resolve persistence threshold
        if min_persistence_frames is None:
            try:
                self.min_persistence_frames = int(os.environ.get("EVENT_MIN_FRAMES", "3"))
            except ValueError:
                self.min_persistence_frames = 3
        else:
            self.min_persistence_frames = min_persistence_frames

        # 2. Resolve minimum confidence
        if min_confidence is None:
            try:
                self.min_confidence = float(os.environ.get("EVENT_CONFIDENCE_THRESHOLD", "0.35"))
            except ValueError:
                self.min_confidence = 0.35
        else:
            self.min_confidence = min_confidence

        # 3. Resolve suppression & cooldown
        cooldown = event_cooldown_seconds if event_cooldown_seconds is not None else time_suppression_seconds
        if cooldown is None:
            try:
                self.time_suppression_seconds = float(os.environ.get("TIME_SUPPRESSION_SECONDS", "30.0"))
            except ValueError:
                self.time_suppression_seconds = 30.0
        else:
            self.time_suppression_seconds = cooldown

        if spatial_suppression_meters is None:
            try:
                self.spatial_suppression_meters = float(os.environ.get("SPATIAL_SUPPRESSION_METERS", "15.0"))
            except ValueError:
                self.spatial_suppression_meters = 15.0
        else:
            self.spatial_suppression_meters = spatial_suppression_meters

        # 4. Resolve evidence storage path
        if evidence_storage_path is None:
            self.evidence_storage_path = os.environ.get("EVIDENCE_STORAGE_PATH", "runtime/evidence")
        else:
    ...
    def process_tracks(
        ...
        for track in tracks:
            ...
            # Trigger event candidate when persistence criteria and confidence threshold met and not already emitted
            if obs.hits >= self.min_persistence_frames and obs.best_confidence >= self.min_confidence and not obs.event_emitted:
                ...
                if not self._is_suppressed(eff_lat, eff_lon, obs.class_name):
                    ...
                    # Save evidence image
                    storage_base = self.evidence_storage_path.rstrip("/")
                    evidence_rel_path = f"{storage_base}/{event_id}.jpg"
                    if obs.best_frame is not None:
                        try:
                            ...
                            logger.error("Failed capturing evidence image for %s: %s", event_id, e)
