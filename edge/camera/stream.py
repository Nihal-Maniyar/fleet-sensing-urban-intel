...
class VideoStreamSource:
    ...
    def __init__(self, width: int = 640, height: int = 480, fps: float = 20.0) -> None:
        self.width = width
        self.height = height
        self.fps = fps
        self._frame_index = 0
...
class SyntheticRoadStream(VideoStreamSource):
    ...
    def __init__(
        ...
        super().__init__(width=width, height=height, fps=float(fps))
    ...
    def read(self) -> Tuple[bool, np.ndarray]:
        ...
        self._frame_count += 1
        self._frame_index = self._frame_count
        now = time.time()
...
class OpenCVFileStream(VideoStreamSource):
    """Reads from a video file (.mp4, .avi) or hardware webcam index sequentially."""
    ...
    def __init__(self, source: str | int, loop: bool = True) -> None:
        ...
        fps = float(raw_fps) if raw_fps and raw_fps > 0 and not np.isnan(raw_fps) else 25.0
        width = int(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        height = int(self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
        total_frames = max(0, int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT)))
    ...
    def read(self) -> Tuple[bool, np.ndarray]:
        ...
        if not ret:
            if self.loop and self.total_frames > 0:
                ...
                ret, frame = self.cap.read()
                self._frame_index = 1 if ret else 0
            else:
                return False, np.zeros((self.height or 480, self.width or 640, 3), dtype=np.uint8)
        else:
            self._frame_index += 1
        return ret, frame
